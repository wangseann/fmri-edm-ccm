...
FEATURES_EVAL_BASE=${FEATURES_EVAL_BASE:-features_day26_ridge_eval}
FIGS_BASE=${FIGS_BASE:-}
...
CMD=("python" "${SCRIPT_DIR}/day26_smoothing_ridge.py"
    ...
    --ccm-samples "${CCM_SAMPLES}"   # arg exists but is ignored inside ridge
...
printf 'Running Day26 Ridge smoothing sweep with command:\n  %s\n' "${CMD[*]}"
"${CMD[@]}"