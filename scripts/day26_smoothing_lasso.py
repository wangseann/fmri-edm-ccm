...
from sklearn.linear_model import Lasso  # CHANGED: was Ridge
...
# ---------------------------------------------------------------------
# Ridge-specific helpers (now using LASSO under the hood)
# ---------------------------------------------------------------------
...
def run_ridge_no_lag(
    *,
    ...
    # Wide log grid to reduce floor/ceiling hits
    ...
    model = Lasso(alpha=best_alpha if best_alpha is not None else 1.0, fit_intercept=True, max_iter=10000)
    model.fit(X_train_std, y_train)
