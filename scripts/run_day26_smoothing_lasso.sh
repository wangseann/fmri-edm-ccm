...
CCM_SAMPLES=${CCM_SAMPLES:-${SAMPLE_STEPS:-10}}   # kept for CLI compat but unused
FEATURES_EVAL_BASE=${FEATURES_EVAL_BASE:-features_day26_lasso_eval}
FIGS_BASE=${FIGS_BASE:-}
...
# NOTE: we still call day26_smoothing_ridge.py — it now runs LASSO internally.
CMD=("python" "${SCRIPT_DIR}/day26_smoothing_ridge.py"
    --config "${CONFIG}"
    --subject "${SUBJECT}"
    --story "${STORY}"
    --target "${TARGET}"
    ...
    --ccm-samples "${CCM_SAMPLES}"   # arg exists but is ignored inside ridge/lasso
...
printf 'Running Day26 LASSO smoothing sweep with command:\n  %s\n' "${CMD[*]}"
"${CMD[@]}"